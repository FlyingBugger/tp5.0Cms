...
class Index extends Common
{
    ...
    public function newpage()
    {
        ...
        if(Request::instance()->has('biaoti','post') && $this->verification())
        {
            ...
            $data = ['post_author' => Session::get($this->session_prefix.'user_id'), 'post_keywords' => htmlspecialchars($guanjianci), 'post_date' => Request::instance()->post('fabushijian'), 'post_content' => $neirong, 'post_title' => htmlspecialchars(Request::instance()->post('biaoti')), 'post_excerpt' => htmlspecialchars(Request::instance()->post('zhaiyao')), 'post_modified' => Request::instance()->post('fabushijian'), 'post_type' => 1, 'thumbnail' => $suolvetu, 'template' => Request::instance()->post('template')];
    ...
    public function editpage()
    {
        ...
        if(Request::instance()->has('postId','post') && $this->verification())
        {
            ...
            $data = ['post_author' => Session::get($this->session_prefix.'user_id'), 'post_keywords' => htmlspecialchars($guanjianci), 'post_content' => $neirong, 'post_title' => htmlspecialchars(Request::instance()->post('biaoti')), 'post_excerpt' => htmlspecialchars(Request::instance()->post('zhaiyao')), 'post_modified' => date("Y-m-d H:i:s"), 'thumbnail' => $suolvetu, 'template' => Request::instance()->post('template')];
            Db::name('posts')
                ->where('id', Request::instance()->post('postId'))
                ->update($data);
        }
