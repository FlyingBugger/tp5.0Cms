    ...
    <h4>
        ...
        <?php echo lang('Catfish'); ?> <?php echo $version['description']; ?> <?php echo lang('Setup Wizard'); ?><span class="pull-right"><?php echo $version['number']; ?></span>
            ...
            <div class="panel-body">
                <textarea class="form-control" rows="15">Catfish(鲶鱼) CMS软件使用协议
...
许可您的权利：
    ...
    您可以在完全遵守本最终用户授权协议的基础上，将Catfish(鲶鱼) CMS应用于非商业用途或个人网站，而不必支付软件版权授权费用。
    您可以在协议规定的约束和限制范围内根据需要对Catfish(鲶鱼) CMS的主题进行必要的修改和美化，以适应您的网站要求。
    您可以在协议规定的约束和限制范围内根据需要制作Catfish(鲶鱼) CMS的插件用来扩展功能，以适应您的网站要求。
    您不能修改Catfish(鲶鱼) CMS的程序主体，以及程序中包含的任何Catfish(鲶鱼) CMS相关版权字样。
    您拥有使用Catfish(鲶鱼) CMS构建的网站中的全部内容的所有权，并独立承担与内容相关的法律义务。
    获得商业授权之后，您可以将Catfish(鲶鱼) CMS应用于商业用途，同时依据所购买的授权类型中确定的技术支持服务等级、期限、服务方式和服务内容，自购买时刻起，在技术支持服务期限内拥有通过指定的方式获得指定范围内的技术支持服务。

约束和限制：

    未获商业授权之前，不得将Catfish(鲶鱼) CMS用于商业用途（包括但不限于企业网站、政府、金融、教育机构、学校、社会团体及其他经营性网站、以营利为目或实现盈利的网站，个人网站永久免费且不限制用途）。
...
权利和所有权的保留
    ...
    您一旦安装使用Catfish(鲶鱼) CMS，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。</textarea>
                <br>
                <div class="row text-center">
                    <a href="install/Index/step1"><button type="button" class="btn btn-primary"><?php echo lang('Accept'); ?></button></a>
                </div>
            </div>
        </div>
    </div>
